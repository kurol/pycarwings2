...
class CarwingsResponse:
    ...
    def _set_cruising_ranges(
            self,
            status,
            off_key="cruisingRangeAcOff",
            on_key="cruisingRangeAcOn"):
...
class CarwingsBatteryStatusResponse(CarwingsResponse):
    def __init__(self, status):
        ...
        # 2016-12: battery degradation is always 0-12 even if battery capacity
        # is diminished.
...
class CarwingsStartClimateControlResponse(CarwingsResponse):
    def __init__(self, status):
        ...
        self.ac_continue_time = timedelta(
            minutes=float(status["acContinueTime"]))
...
class CarwingsDrivingAnalysisResponse(CarwingsResponse):
    def __init__(self, status):
        ...
        # avg energy economy, in units of 'electric_cost_scale' (e.g.
        # miles/kWh)
...
class CarwingsLatestBatteryStatusResponse(CarwingsResponse):
    def __init__(self, status):
        self.answer = status
        if not "BatteryStatusRecords" in status:
            return        
        ...
        # another double negative, for the kids
        ...
        self.battery_percent = 100 * \
            float(self.battery_remaining_amount) / float(self.battery_capacity)

        # Leaf 2016 has SOC (State Of Charge) in BatteryStatus, a more accurate
        # battery_percentage
        if "SOC" in bs:
